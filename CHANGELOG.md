...
- Improve terrain rendering performance by only allocating a tile-ID clone and a matrix for terrain-tile × source-tile pairs that actually overlap. `TerrainTileManager._getTerrainCoordsForRegularTile` allocated both before the overlap test and discarded them for the ~95% of pairs that do not overlap, which cost around 1 ms of main-thread CPU per frame in a draped-terrain scene that repaints continuously
